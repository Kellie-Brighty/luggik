...
export default function RunnerSignup() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // After Google auth, check KYC status
      ...
      if (res.ok) {
        ...
        // New user without a profile, redirect to profile completion
    ...
    <div className="min-h-screen bg-slate-50 font-sans p-6 flex flex-col items-center justify-center">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Logistics Partner</h1>
          <p className="text-slate-500 mt-1">Create an account to accept deliveries</p>
        </div>
        ...
        <div className="mt-6 text-center text-sm text-slate-500">
          Already have an account? <Link to="/runner/login" className="text-nomba-dark font-semibold hover:underline">Log in</Link>
        </div>
      </div>
    </div>
  );
